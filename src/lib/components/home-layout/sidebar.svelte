...
<aside class="flex h-full flex-col p-2">
	<a class="m-2 flex gap-4 rounded p-2 hover:bg-base-200" href={resolve('/')}>
		<!-- Home icon -->
		<img src={logo} alt="Logo" class="max-h-12 max-w-12" />
		<div>
			<h1 class="text-lg font-bold">{m.title()}</h1>
			<span class="text-sm">{m.admin_dashboard()}</span>
		</div>
	</a>
